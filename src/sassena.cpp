...
//	}
	...
	for (vector<CartesianCoor3D>::iterator qqqi=myqqqvectors.begin();qqqi!=myqqqvectors.end();qqqi++) {
		qqqcounter++;
			...
			sample.atoms.push_scatteramps(target); // now atoms has a continuous field for scatteramps
...
//							scattering_amplitudes.push_back(scatsuperposed);
							...
							if (avmethod=="debye") {
								Analysis::scatter_sphere_debye(sample,target_selection,*qqqi,resolution,scattering_amplitudes);
							}
						}
						else if (avtype=="cylinder") {
							string avmethod = params->scattering.average.method;
...
//							for(size_t asqi = 0; asqi < qvectors.size(); asqi++)
//							{
//								// write frames into static array
//								for(int fn = 0; fn < myframes.size(); ++fn)
//								{
...
//								timer.start("scatter::agg::correlate");
//								
//								// barrier to remove load imbalance from performance measure
//								if (params->debug.barriers) {
//									timer.start("scatter::agg::corr::barrier");
//									local.barrier();
//									timer.stop("scatter::agg::corr::barrier");
...
//							}
							...
							for(size_t asqi = 0; asqi < qvectors.size() ; ++asqi) {
								...
								for(size_t fi = 0; fi < sample.frames.size(); ++fi) {
									...
									size_t frame_offset = ( (fi*local.size()) % sample.frames.size() / local.size());
									size_t pos = ( (rank_of_frame*my_aqs_max_size) + frame_offset*2 ); 
									// add the complex scattering amplitude to the current ones
									aq_vectors[fi]+=complex<double>(all_aqs[pos],all_aqs[pos+1]); 
								}
							}
