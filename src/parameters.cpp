...
void Database::read_conf(std::string filename) {
	...
	if (rootsetting["names"].exists("pdb")) {
		for (int i=0;i<rootsetting["names"]["pdb"].getLength();i++) {	
			string label = getstring(rootsetting["names"]["pdb"][i].getName());
			string regexp = getstring(rootsetting["names"]["pdb"][i]);
			names.pdb.reg(label,regexp);
		}		
	}

	for (int i=0;i<rootsetting["masses"].getLength();i++) {	
		string label = getstring(rootsetting["masses"][i].getName());
		double mass = getdouble(rootsetting["masses"][i]);
	...
	for (int i=0;i<rootsetting["sizes"].getLength();i++) {	
		...
		string label = getstring(rootsetting["sizes"][i].getName());
		size_t sizes_function_type = getlong(rootsetting["sizes"][i][0]);
		
		vector<double> values;
		for(size_t k = 1; k < rootsetting["sizes"][i].getLength(); ++k)
		{
			values.push_back(getdouble(rootsetting["sizes"][i][k]));
		}					
	...
	for (int i=0;i<rootsetting["sfactors"].getLength();i++) {	
		if (getstring(rootsetting["sfactors"][i].getName())== Params::Inst()->scattering.probe) {
		
			for (int j=0;j<rootsetting["sfactors"][i].getLength();j++) {	
				
				string label = getstring(rootsetting["sfactors"][i][j].getName());
				size_t sfactors_function_type = getlong(rootsetting["sfactors"][i][j][0]);
				size_t ID = atomIDs.get(label);
				
				vector<double> values;
				for(size_t k = 1; k < rootsetting["sfactors"][i][j].getLength(); ++k)
				{
					values.push_back(getdouble(rootsetting["sfactors"][i][j][k]));
				}					
